...
import type { EffectPreset, TransitionType } from "./index";
...
export type ModulationSourceType = "lfo" | "audio";

export type AudioSourceBand =
  | "bass"
  | "lowMid"
  | "mid"
  | "highMid"
...
export type ModulationFieldType =
  | "number"
  | "vec-component"
  | "color-hue"
  | "select"
...
export interface ModulationConfig {
  id: string;
  layerId: string;
  /** Dot-notation path into layer config, e.g. "opacity", "rotation.0", "uniforms.uScale.value" */
  propertyPath: string;
  /** Human-readable name shown in card header */
  propertyLabel: string;
  /** What kind of field this modulates */
  fieldType: ModulationFieldType;
  /** Source type: LFO oscillator or audio band */
  sourceType: ModulationSourceType;
  /** LFO waveform shape */
  shape: LFOShape;
  /** LFO frequency in Hz (0.033 = 30s period, 10 = 0.1s period) */
  frequency: number;
  /** Phase offset 0–1 */
  phase: number;
  /** Audio band to use when sourceType='audio' */
  audioBand: AudioSourceBand;
  /** Output amplitude (scale factor 0–1) */
  amplitude: number;
  /** DC offset (–1 to +1) */
  offset: number;
  /** Whether this modulation is active */
  enabled: boolean;
  /** For select cycling: ordered list of values to cycle through */
  selectValues?: string[];
}
...
export interface InstancedParticlesLayer extends LayerBase {
  ...
  color: string;
  attractors: Array<
    { position: [number, number, number]; strength: number; radius: number }
  >;
  damping: number;
  maxSpeed: number;
  audioReactive: boolean;
